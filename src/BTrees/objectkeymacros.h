...
/* Note that the second comparison is skipped if the first comparison returns:
  ...
  -1 -> There was an error, which the caller will detect with
        PyError_Occurred.
 */
...
check_argument_cmp(PyObject *arg)
{
    ...
    /*        (&PyBaseObject_Type)->ob_type->tp_richcompare, */
    /*        arg->ob_type->tp_compare, */
    /*        (&PyBaseObject_Type)->ob_type->tp_compare); */
    if (arg == Py_None) {
      return 1;
    }


    if (Py_TYPE(arg)->tp_richcompare == (&PyBaseObject_Type)->tp_richcompare)
    {
