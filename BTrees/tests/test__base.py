...
class Test_BucketBase(unittest.TestCase):
    ...
    def test_iterkeys_w_min_hit_w_exclude_min(self):
        ...
        bucket._to_key = lambda x: x
        KEYS = bucket._keys = ["alpha", "bravo", "charlie", "delta", "echo"]
        self.assertEqual(list(bucket.iterkeys(min="bravo", excludemin=True)), KEYS[2:5])
    ...
    def test_iterkeys_w_min_miss_w_exclude_min(self):
        ...
        KEYS = bucket._keys = ["alpha", "bravo", "charlie", "delta", "echo"]
        # 'excludemin' doesn't fire on miss
        self.assertEqual(list(bucket.iterkeys(min="candy", excludemin=True)), KEYS[2:5])
    ...
    def test_iterkeys_w_max_hit_w_exclude_max(self):
        ...
        bucket._to_key = lambda x: x
        KEYS = bucket._keys = ["alpha", "bravo", "charlie", "delta", "echo"]
        self.assertEqual(list(bucket.keys(max="delta", excludemax=True)), KEYS[0:3])
    ...
    def test_iterkeys_w_max_miss_w_exclude_max(self):
        ...
        KEYS = bucket._keys = ["alpha", "bravo", "charlie", "delta", "echo"]
        # 'excludemax' doesn't fire on miss
        self.assertEqual(list(bucket.iterkeys(max="dandy", excludemax=True)), KEYS[0:3])
...
class BucketTests(unittest.TestCase):
    ...
    def test_keys_filled_no_args(self):
        ...
        for i, c in enumerate("abcdef"):
            bucket[c] = i
        self.assertEqual(bucket.keys(), ["a", "b", "c", "d", "e", "f"])
    ...
    def test_iterkeys_filled_w_args(self):
        ...
        for i, c in enumerate("abcdef"):
            bucket[c] = i
        self.assertEqual(
            list(bucket.iterkeys(min="b", excludemin=True, max="f", excludemax=True)),
            ["c", "d", "e"],
        )
    ...
    def test_values_filled_w_args(self):
        ...
        for i, c in enumerate("abcdef"):
            bucket[c] = i
        self.assertEqual(
            bucket.values(min="b", excludemin=True, max="f", excludemax=True), [2, 3, 4]
        )
    ...
    def test_itervalues_filled_w_args(self):
        ...
        for i, c in enumerate("abcdef"):
            bucket[c] = i
        self.assertEqual(
            list(bucket.itervalues(min="b", excludemin=True, max="f", excludemax=True)),
            [2, 3, 4],
        )
    ...
    def test_iteritems_filled_w_args(self):
        ...
        for i, c in enumerate("abcdef"):
            ...
            EXPECTED.append((c, i))
        self.assertEqual(
            list(bucket.iteritems(min="b", excludemin=True, max="f", excludemax=True)),
            EXPECTED[2:5],
        )
    ...
    def test__p_resolveConflict_x_on_com_next_old_new_None(self):
        ...
        s_new = None
        e = self.assertRaises(
            BTreesConflictError, bucket._p_resolveConflict, s_old, s_com, s_new
        )
...
class SetTests(unittest.TestCase):
    ...
    def test__p_resolveConflict_x_on_com_next_old_new_None(self):
        ...
        s_new = None
        e = self.assertRaises(
            BTreesConflictError, _set._p_resolveConflict, s_old, s_com, s_new
        )
        self.assertEqual(e.reason, 0)
...
class Test_Tree(unittest.TestCase):
    ...
    def test_keys_w_min_hit_w_exclude_min(self):
        ...
        for key in KEYS:
            tree[key] = key.upper()
        self.assertEqual(list(tree.keys(min="bravo", excludemin=True)), KEYS[2:5])
    ...
    def test_keys_w_min_miss_w_exclude_min(self):
        ...
        for key in KEYS:
            tree[key] = key.upper()
        # 'excludemin' doesn't fire on miss
        self.assertEqual(list(tree.keys(min="candy", excludemin=True)), KEYS[2:5])
    ...
    def test_keys_w_max_hit_w_exclude_max(self):
        ...
        for key in KEYS:
            tree[key] = key.upper()
        self.assertEqual(list(tree.keys(max="delta", excludemax=True)), KEYS[0:3])
    ...
    def test_keys_w_max_miss_w_exclude_max(self):
        ...
        for key in KEYS:
            tree[key] = key.upper()
        # 'excludemax' doesn't fire on miss
        self.assertEqual(list(tree.keys(max="dandy", excludemax=True)), KEYS[0:3])
    ...
    def test__p_resolveConflict_invalid_state_non_tuple(self):
        ...
        EMPTY = None
        DEGEN = (((("a", "b"),),),)
        self.assertRaises(TypeError, tree._p_resolveConflict, INVALID, EMPTY, DEGEN)
        self.assertRaises(TypeError, tree._p_resolveConflict, EMPTY, INVALID, DEGEN)
        self.assertRaises(TypeError, tree._p_resolveConflict, EMPTY, DEGEN, INVALID)
    ...
    def test__p_resolveConflict_non_degenerate_state(self):
        ...
        DEGEN = (((("a", "b"),),),)
        e = self.assertRaises(
            BTreesConflictError, tree._p_resolveConflict, NON_DEGEN, EMPTY, DEGEN
        )
        self.assertEqual(e.reason, 11)
        e = self.assertRaises(
            BTreesConflictError, tree._p_resolveConflict, EMPTY, NON_DEGEN, DEGEN
        )
        self.assertEqual(e.reason, 11)
        e = self.assertRaises(
            BTreesConflictError, tree._p_resolveConflict, EMPTY, DEGEN, NON_DEGEN
        )
        self.assertEqual(e.reason, 11)
...
class TreeTests(unittest.TestCase):
    ...
    def test_values_filled_w_args(self):
        ...
        tree = self._makeOne(ITEMS)
        self.assertEqual(
            list(tree.values(min="b", excludemin=True, max="f", excludemax=True)),
            [2, 3, 4],
        )
    ...
    def test_itervalues_filled_w_args(self):
        ...
        tree = self._makeOne(ITEMS)
        self.assertEqual(
            list(tree.itervalues(min="b", excludemin=True, max="f", excludemax=True)),
            [2, 3, 4],
        )
    ...
    def test_items_filled_w_args(self):
        ...
        tree = self._makeOne(ITEMS)
        self.assertEqual(
            list(tree.items(min="b", excludemin=True, max="f", excludemax=True)),
            ITEMS[2:5],
        )
    ...
    def test_iteritems_filled_w_args(self):
        ...
        tree = self._makeOne(ITEMS)
        self.assertEqual(
            list(tree.iteritems(min="b", excludemin=True, max="f", excludemax=True)),
            ITEMS[2:5],
        )
    ...
    def test_byValue(self):
        ...
        tree = self._makeOne(ITEMS)
        self.assertEqual(
            list(tree.byValue(min=22)), [(y, x) for x, y in reversed(ITEMS[22:])]
        )
